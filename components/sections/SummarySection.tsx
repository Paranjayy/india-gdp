    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          <div key={stat.label} className="bg-card rounded-2xl border border-hairline p-5 shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
            <div className="text-[11px] text-muted mb-2 tracking-tight">{stat.label}</div>
      ...
      <div>
        <div className="text-[12px] font-medium text-muted mb-3">India relative to major economies (Nominal GDP)</div>
          ...
          {[
            ...
            return (
              <div key={country.iso3} className="flex items-center gap-3 group">
                <div className="w-24 text-[11px] text-muted shrink-0 flex items-center gap-1.5">
                  <span>{country.flag}</span>
                  <span className="truncate">{label}</span>
                </div>
                <div className="flex-1 relative h-5 bg-bg rounded-full overflow-hidden">
                  <div
                  ...
                  {/* India bar */}
                  <div
                    className="absolute inset-y-0 rounded-full bg-india-saffron/20"
                    style={{ width: "100%", border: "1px dashed #FF9933" }}
                  />
                </div>
                <div className="text-[11px] tabular-nums w-14 text-right text-muted">
                  {ratio.toFixed(1)}%
                </div>
                <div
      ...
      <div className="bg-orange-50/50 dark:bg-orange-950/20 border border-orange-200/50 dark:border-orange-900/30 rounded-2xl p-5 flex items-start gap-4">
        ...
        <div>
          ...
          <div className="text-[13px] text-muted leading-relaxed">
            ...
            a net <span className="highlight-sweep font-semibold text-ink">+3.9% on 2019 levels</span> within 2 years.
